...
class RouletteEuropeenneView(discord.ui.View):
    def __init__(self, user):
        ...
        # Limite à 25 options (test temporaire)
        options = [
            discord.SelectOption(label=str(n), description=f"Parier sur le numéro {n}") 
            for n in range(1,25)
        ]

        # Ajout d'un custom_id + sécurité
        self.select = Select(
            placeholder="Choisissez un numéro sur lequel parier (0-24)",
            options=options,
    ...
    async def select_callback(self, interaction: discord.Interaction):
        try:
            ...
            self.bet_number = int(self.select.values[0])
            self.spin_button.disabled = False
            print(f"[DEBUG] {interaction.user} a sélectionné {self.bet_number}")
            await interaction.response.edit_message(
                content=f"Tu as parié sur le numéro {self.bet_number}. Lance la roulette !",
                view=self
            )
        except Exception as e:
            print(f"[ERREUR SELECT CALLBACK] {e}")
            await interaction.response.send_message(f"Erreur dans select : {e}", ephemeral=True)
    ...
    async def spin_callback(self, interaction: discord.Interaction):
        try:
            ...
            print(f"[DEBUG] Roulette résultat : {result} (pari sur {self.bet_number})")

            if result == self.bet_number:
                description = f"🎉 La roulette s'arrête sur **{result}** !\nFélicitations, tu as gagné ton pari ! 🏆"
                color = discord.Color.green()
            else:
                description = f"❌ La roulette s'arrête sur **{result}**.\nDommage, tu as perdu ton pari {self.bet_number}."
                color = discord.Color.red()
            ...
            print(f"[ERREUR SPIN CALLBACK] {e}")
            await interaction.response.send_message(f"Erreur dans spin : {e}", ephemeral=True)
