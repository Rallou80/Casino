...
from discord.ui import View, Button  # Import manquant corrigé
import random
import threading
...
class RouletteEuropeenneView(discord.ui.View):
    def __init__(self, user):
        ...
        # Ajout du select de mise ici
        options = [discord.SelectOption(label=str(n), description=f"Parier sur le numéro {n}") for n in range(37)]
        self.select = Select(placeholder="Choisissez un numéro sur lequel parier (0-36)", options=options)
    ...
    async def spin_callback(self, interaction: discord.Interaction):
        ...
        self.stopped = True
        # On désactive les boutons après le spin
